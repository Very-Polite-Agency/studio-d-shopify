...
const Scrolling = (() => {
  ...
  const abc = () => {
    ...
    document.querySelectorAll( '.js--in-view' ).forEach( element => {
      if ( elementIsInView( element ) ) {
        tools.addClass( 'in-view', [ element ] );
        tools.removeClass( 'not-in-view', [ element ] );
      } else {
        tools.addClass( 'not-in-view', [ element ] );
        tools.removeClass( 'in-view', [ element ] );
      }
    });
